...
import { baseURL, externalServiceURL, convertToBase64, mode, payment, webRequest } from '../global';
...
import { setMaticBalance } from '../state/Profile/ProfileSlice';
...
const PackageForm: React.FC = () => {
    ...
    const handleFormSubmission = async () => {
        ...
        try {
            ...
            if (data === "ACCEPTED") {
                dispatch(setMaticBalance(maticBalance)); // just to show that the fee deduction section could be executed  - computeMatic(formData.packageLength, formData.packageWidth, formData.packageHeight)
                // TODO: Payment
                // if (mode !== "p") {
                //     dispatch(setMaticBalance(maticBalance)); // just to show that the fee deduction section could be executed  - computeMatic(formData.packageLength, formData.packageWidth, formData.packageHeight)
                // }
                // payment(wallet_address, wallet_key, "root", "ADMIN", computeMatic(formData.packageLength, formData.packageWidth, formData.packageHeight).toString()) //exempted
                //     .then(
                //         h => {
    ...
    return (
        <div className="flex flex-col items-center bg-hero-pattern bg-gray-400">
            ...
            <div className="container bg-gray-200 shadow-2xl rounded-xl my-16 py-2">
                {formData.packageId && <>
                ...
                <div id="map" className='hover:scale-150 transition duration-300 ease-in-out'>
                ...
                <form onSubmit={handleSubmit} className="flex flex-col justify-center items-center">
                    {/* <div className='flex flex-row'>
                        <h5 className='m-2'>Avatar:</h5>
                        <input className='block w-full px-4 py-2 pr-8 leading-tight bg-white border border-gray-300 rounded-md shadow-sm appearance-none focus:outline-none focus:border-blue-500 hover:scale-110 duration-300 transition ease-in-out'
                            type="file"
                            name='avatar'
                    ...
                    <input
                        className='block w-full px-4 py-2 pr-8 leading-tight bg-white border border-gray-300 rounded-md shadow-sm appearance-none focus:outline-none focus:border-blue-500 hover:scale-110 duration-300 transition ease-in-out'
                        type="text"
                        name="time"
                        placeholder="time"
                        disabled={true}
                    ...
                    <input
                        ...
                        onChange={handleChange}
                    />
                    <h5 className='border border-dotted border-gray-500 p-2 my-2'>Click <text onClick={() => { navigate("/size") }} className='text-blue-700 underline'>here</text> to upload images and detect sizes.</h5>
                    <div className='flex flex-row'>
                    ...
                    <select
                        className='block w-full mb-2 px-4 py-2 pr-8 leading-tight bg-white border border-gray-300 rounded-md shadow-sm appearance-none focus:outline-none focus:border-blue-500 hover:scale-110 duration-300 transition ease-in-out'
                ...
                <button className="bg-gray-200 hover:bg-gray-300 text-gray-800 font-semibold py-2 px-4 border border-gray-400 duration-300 ease-in-out" onClick={handleGoBackNavigation}>Go Back</button>
            </div>
        </div>
    );
};
