...
const TrackPage = () => {
    ...
    // const [packages, setPackages] = useState<Package[]>(p2pInfo.concat(w2pInfo));
    ...
    const fetchPackages = async () => {
        ...
        try {
            ...
            const w2p = await w2pRes.json();
            console.log(w2pInfo);

            const allInfo = p2p.concat(w2p);
            setPackages(allInfo);
        } catch (error) {
            console.error(error);
        } finally {
    ...
    const filterByID = (value: React.SetStateAction<string | number | readonly string[] | undefined>) => {
        ...
        setPackagesFiltered(packagesFiltered.filter(x => x.package_id.includes(packageIDFilter!.toString())));
    }
                    ...
                    <div className='flex flex-col m-8 flex-wrap bg-gray-200 p-8 rounded-xl shadow-xl overflow-auto'>
                        ...
                        <div className='flex flex-row justify-between'>
                            <div className='grid grid-cols-2 gap-2'>
                                ...
                                <div className="transition form-group hover:scale-110 duration-300">
                                    <input className="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 hover:placeholder-gray-500" type="number" placeholder="Filter By Dimension Value (Only Packages With L * W * H Larger Than The Given Value Will Show)" value={packageDimensionsFilter} onChange={(e) => filterByDimension(e.target.value)} />
                                </div>
                            ...
                            {packageIDFilter === "" && packageSenderName === "" && packageDimensionsFilter === 0 && onlyShowFinished === false ? packages.map(
                                packageItem => {
                                    ...
                                    return <li className="hover:scale-105 transition duration-300 ease-in-out" key={packageItem.package_id + Math.random().toString()}>
                                        <PackageAvatar id={packageItem.package_id + ": " + packageItem.package_description} description={packageItem.package_sender_name ? "From " + packageItem.package_sender_name : "From the warehouse"} jsonified={JSON.stringify(packageItem)} handler={HandlePackageClickedNavigation}></PackageAvatar>
                                    </li>
                                }
