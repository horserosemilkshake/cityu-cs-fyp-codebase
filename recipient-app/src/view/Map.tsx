...
const Map = () => {
    ...
    async function calculateETA() {
        ...
        for (let i = 0; i < result.length - 1; i++) {
            ...
            let estKm = haversineDistance(x0, y0, x1, y1) / 1000;  //estimate
    ...
    function calculateSpeed(newCoord: string) {
        if (correspondingDriverCoordinate === "") {
            setCorrespondingDriverCoordinate(newCoord);
        }
        let x0 = +correspondingDriverCoordinate.split(",")[0];
        let y0 = +correspondingDriverCoordinate.split(",")[1];
        let x1 = +newCoord.split(",")[0];
        let y1 = +newCoord.split(",")[1];

        let estKm = haversineDistance(x0, y0, x1, y1) / 1000;  //estimate

        return (estKm / 5) * 60 * 60;
    }
                    ...
                    response.json().then(function (data) {
                        ...
                        for (let i in temp) {
                            if (target != null && i === target.responsible_driver_name) {
                                setSpeed(calculateSpeed(temp[i]["coordinate"]));
    ...
    return (
        <div className="flex flex-col items-center bg-hero-pattern bg-gray-400">
            ...
            <div className='container bg-gray-200 opacity-90 shadow-2xl p-4 rounded-xl mt-16 mb-256'>
                ...
                <div className='flex flex-row items-center my-8 w-full'>
                    <div className='flex-1 mx-4 w-full'>
                        {(!target?.finished) ?
                            <Table data={[
                                ...
                                {
                                    label: "Speed (km/h)",
                                    value: correspondingDriver ? String((Math.abs(speed)).toFixed(2)) : "Not Ready",
                                },
                                {
...

                    {
                        (latGlobal === 0 || lonGlobal === 0) ?
                            <h1>Loading...</h1> :
                            <>
                                <div id="map" key="map" className='border border-gray-400 hover:scale-150 duration-300 ease-in-out mr-4'>
                                    <MapContainer className="" center={[latGlobal, lonGlobal]} zoom={13} scrollWheelZoom={false}>
                                        <TileLayer
                                            attribution='&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors'
                                            url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png"
                                        />
                                        {(target?.finished) && <RoutingMachine key="client_route" color={"#FF0000"} source={[latGlobal, lonGlobal]} destination={[+packageData.package_destination_coordinate.split(",")[0], +packageData.package_destination_coordinate.split(",")[1]]} />}
                                        <RoutingMachine key="pickup_to_delivery_route" color={"#6FA1EC"} source={[+packageData.package_pickup_coordinate.split(",")[0], +packageData.package_pickup_coordinate.split(",")[1]]} destination={[+packageData.package_destination_coordinate.split(",")[0], +packageData.package_destination_coordinate.split(",")[1]]} />
                                        {(correspondingDriverRoute.length !== 0) ? <RoutingMachine key="deliverer_route" color={"#ECF542"} sequence={correspondingDriverRoute} destination={[+packageData.package_destination_coordinate.split(",")[0], +packageData.package_destination_coordinate.split(",")[1]]} /> : <></>}
                                        ...
                                        {(correspondingDriverCoordinate !== "") ?
                                            <>
                                                <Marker position={[+correspondingDriverCoordinate.split(",")[0], +correspondingDriverCoordinate.split(",")[1]]} icon={DefaultIcon}>
                                                    <Popup>
                                                        Latest Deliverer Position.
                                                    </Popup>
                                                </Marker>
                                                <MarkerLayer>
                                                    <RLM
